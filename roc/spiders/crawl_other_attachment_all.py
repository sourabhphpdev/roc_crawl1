...
class CrawlOtherAttachmentAll(scrapy.Spider):
    name = 'crawl_other_attachment_all'
    today = datetime.now()
    ...
    def parse(self, response):
        ...
        if not os.path.exists(src):
            ...
            with open(src + '/cin_list_other_attachment.csv', 'r') as file:
        ...
        # cin_list=['U72900KA2011PTC060958','U72900DL2011PTC225614']
                        ...
                        if err==0:
                            ...
                            if not_availabe_text:
                                try:
                                    ...
                                    temp = ''+err
                                    self.save_to_api_attachment(cin, False, temp)
                                    fields = [cin, 'Success', temp]
                                    with open(src + '/cin_list_other_attachment.csv', 'a') as f:
                                        writer = csv.writer(f)
                                        writer.writerow(fields)
                                ...
                                try:
                                    ...
                                    work['Doc_Proc']=self.parse_edoc(html, cin)
                                    ar_text='data saved'
    ...
    def parse_edoc(self, html, cin):
        ...
        try:
            ...
            for x, y in zip(res[0::2], res[1::2]):
                temp += x.strip() + ' : ' + y.strip() + '<br>'
            self.save_to_api_attachment(cin, True, temp)
            return 'Work Completed'
        except Exception as e:
            temp = 'no response found today'
            self.save_to_api_attachment(cin, False, temp)
    ...
    def start_driver(self,url):
        ...
        # PROXY = "159.89.113.32:3128"  # IP:PORT or HOST:PORT
    ...
    def save_to_api_attachment(self, cin, doc_present, doc_info):

        url = "http://52.66.198.92/api/mca_info/cinMarked/"
        if doc_present == True:
            payload = "cinNumer=" + cin + "&other_attachment_documents_present=True&other_attachment_documents_information=" + doc_info
            headers = {
                'content-type': "application/x-www-form-urlencoded",
            }
            requestto = requests.request("POST", url, data=payload, headers=headers)
            ...
            payload = "cinNumer=" + cin + "&other_attachment_documents_present=False&other_attachment_documents_information=" + doc_info
            headers = {
                'content-type': "application/x-www-form-urlencoded",
            }
            requestto = requests.request("POST", url, data=payload, headers=headers)
